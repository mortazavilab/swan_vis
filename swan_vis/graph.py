...
class Graph:
    def __init__(self):
        ...
        self.loc_df = pd.DataFrame(columns=["chrom", "coord", "vertex_id"])
        self.edge_df = pd.DataFrame(
            columns=["v1", "v2", "strand", "edge_type", "edge_id"]
        )
        self.t_df = pd.DataFrame(columns=["tname", "gid", "gname", "path", "tid"])
    ...
    def get_ordered_id_map(self, rev_strand=False):
        ...
        if rev_strand:
            self.loc_df.sort_values(
                ["chrom", "coord"], ascending=[True, False], inplace=True
            )
        else:
            self.loc_df.sort_values(
                ["chrom", "coord"], ascending=[True, True], inplace=True
            )
    ...
    def get_loc_path(self):
        ...
        self.t_df["loc_path"] = self.t_df.apply(
            lambda x: get_transcript_loc_path(x.path, self.edge_df), axis=1
        )
    ...
    def get_strand_from_gid(self, gid):
        ...
        if self.has_novelty():
            vertex = self.t_df.loc[
                (self.t_df.gid == gid) & (self.t_df.novelty != "Antisense")
            ].path.tolist()[0][0]
        else:
    ...
    def get_chrom_from_gid(self, gid):
        ...
        Parameters:
                gid (str): Gene ID
        """
        chrom = self.loc_df.loc[
            self.t_df.loc[self.t_df.gid == gid, "loc_path"].tolist()[0][0], "chrom"
        ]
        return chrom
