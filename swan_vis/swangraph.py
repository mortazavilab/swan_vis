...
class SwanGraph(Graph):
    ...
    def add_annotation(self, fname, verbose=False):
        ...
        self.add_dataset(fname, include_isms=True, annotation=True, verbose=verbose)
    ...
    def add_transcriptome(
        ...
        tids = self.add_dataset(
            fname, pass_list=pass_list, include_isms=include_isms, verbose=verbose
        )
    ...
    def add_dataset(
        self, fname, pass_list=None, include_isms=False, annotation=False, verbose=False
    ):
        """
        ...
        if ftype == "gtf":
            ...
            t_df, exon_df, from_talon, from_cerberus = parse_gtf(
                fname, include_isms, verbose
            )
        elif ftype == "db":
            check_file_loc(fname, "TALON DB")
            observed = True
            t_df, exon_df = parse_db(fname, pass_list, observed, include_isms, verbose)
            from_talon = True
            from_cerberus = False
        ...
        loc_df, edge_df, t_df = self.create_dfs(
            t_df, exon_df, from_talon, from_cerberus
        )
    ...
    def merge_adata_abundance(self, adata, how="iso"):
        ...
        for d in datasets:
            if d in dataset_list:
                raise ValueError(
                    "Dataset {} already present in the SwanGraph.".format(d)
                )
        ...
        if len(datasets) <= 5:
            print(
                "Adding abundance for datasets {} to SwanGraph.".format(
                    ", ".join(datasets)
                )
            )
        else:
            ...
            print(
                "Adding abundance for datasets {}... (and {} more) to SwanGraph".format(
                    ", ".join(mini_datasets), n
                )
            )
        ...
        if not ab_bool:
            ...
            sg_adata.layers["tpm"] = sparse.csr_matrix(
                calc_tpm(sg_adata, recalc=True).to_numpy()
            )

            if not self.sc and how == "iso":
                print("Calculating PI...")
                sg_adata.layers["pi"] = sparse.csr_matrix(
    ...
    def create_cerberus_adata(self, mode):
        ...
        t_counts[name_col] = (
            t_counts.gname + "_" + t_counts[id_col].str.split("_", expand=True)[1]
        )
    ...
    def create_feat_adata(self, kind):
        ...
        if id_col in self.t_df.columns:
            ...
            t_exp_df = pd.DataFrame.sparse.from_spmatrix(
                columns=obs, data=data, index=tid
            )
            t_exp_df = t_exp_df.merge(
                t_df, how="left", left_index=True, right_index=True
            )

            # merge counts per transcript with end expression
            df = df.merge(t_exp_df, how="left", left_index=True, right_index=True)
            ...
            df["end_gene_num"] = (
                df.sort_values(["gid", "vertex_id"], ascending=[True, True])
                .groupby(["gid"])
                .cumcount()
                + 1
            )
        ...
        if not self.sc:
            adata.layers["pi"] = sparse.csr_matrix(
                calc_pi(adata, adata.var)[0].to_numpy()
            )
    ...
    def create_edge_adata(self):
        ...
        edge_exp_df = edge_exp_df.merge(
            t_exp_df, how="left", left_index=True, right_index=True
        )
        ...
        edge_exp_df = edge_exp_df.merge(
            self.edge_df[["v1", "v2"]], how="left", left_index=True, right_index=True
        )
    ...
    def add_edge_coords(self):
        ...
        edge_df = edge_df.merge(temp, how="left", left_on="v1", right_on="vertex_id")
        edge_df.rename({"coord": "v1_coord"}, axis=1, inplace=True)

        temp = self.loc_df[["coord", "vertex_id"]]
        temp.reset_index(inplace=True, drop=True)

        edge_df = edge_df.merge(temp, how="left", left_on="v2", right_on="vertex_id")
        edge_df.rename({"coord": "v2_coord"}, axis=1, inplace=True)

        return edge_df

    def create_dfs(self, transcripts, exons, from_talon=False, from_cerberus=False):
        """
        ...
        locs = [
            {"chrom": key[0], "coord": key[1], "vertex_id": vertex_id}
            for key, vertex_id in locs.items()
        ]
    ...
    def order_transcripts_subset(self, t_df, order="tid"):
        ...
        # order by tss location
            ...
            t_df = t_df.merge(
                self.t_df[["path", "loc_path"]],
                how="left",
                left_index=True,
                right_index=True,
            )
            t_df["start_coord"] = t_df.apply(
                lambda x: self.loc_df.loc[x.loc_path[0], "coord"], axis=1
            )
            t_df["strand"] = t_df.apply(
                lambda x: self.edge_df.loc[x.path[0], "strand"], axis=1
            )
            ...
            t_df.drop(
                ["start_coord", "strand", "path", "loc_path"], axis=1, inplace=True
            )
        ...
        # order by tes location
            ...
            t_df["end_coord"] = t_df.apply(
                lambda x: self.loc_df.loc[x.loc_path[-1], "coord"], axis=1
            )
            t_df["strand"] = t_df.apply(
                lambda x: self.edge_df.loc[x.path[-1], "strand"], axis=1
            )
            fwd = t_df.loc[t_df.strand == "+"]
            rev = t_df.loc[t_df.strand == "-"]
    ...
    def order_transcripts(self, order="tid"):
        ...
        # order by expression
            ...
            if not self.abundance:
                ...
                temp = pd.DataFrame(
                    index=["tpm"], data=[data], columns=cols
                ).transpose()
        ...
        # order by coordinate of tss
            ...
            self.t_df["start_coord"] = self.t_df.apply(
                lambda x: self.loc_df.loc[x.path[0], "coord"], axis=1
            )
            ...
            if self.loc_df.loc[self.loc_df.index[0], "strand"] == "-":
                ...
                ascending = True
            self.t_df.sort_values(by="start_coord", ascending=ascending, inplace=True)
            self.t_df.drop("start_coord", axis=1, inplace=True)

        # order by coordinate of tes
        elif order == "tes":
            self.t_df["end_coord"] = self.t_df.apply(
                lambda x: self.loc_df.loc[x.path[-1], "coord"], axis=1
            )
            ...
            if self.loc_df.loc[self.loc_df.index[0], "strand"] == "-":
                ...
                ascending = True
            self.t_df.sort_values(by="end_coord", ascending=ascending, inplace=True)
    ...
    def find_ir_genes(self, verbose=False):
        ...
        if "annotation" not in self.edge_df.columns:
            raise Exception(
                "Cannot find novel IR events without " "annotation in SwanGraph."
            )

        edge_ids = self.edge_df.loc[
            (self.edge_df.annotation == False) & (self.edge_df.edge_type == "exon"),
            "edge_id",
        ]
        ...
        for i, eid in enumerate(edge_ids):
            ...
            self.edge_df["tuple_edge_id"] = self.edge_df[["v1", "v2"]].apply(
                tuple, axis=1
            )
                    ...
                    for gid in cand_genes:
                        ...
                        for cand_eid in sub_edges.index:
                            temp_df = cand_g_df[
                                [
                                    cand_eid in path
                                    for path in cand_g_df.path.values.tolist()
                                ]
                            ]
        ...
        print(
            "Found {} novel ir events in {} transcripts.".format(
                len(ir_df.index), len(ir_transcripts)
            )
        )
    ...
    def find_es_genes(self, verbose=False):
        ...
        edge_ids = self.edge_df.loc[
            (self.edge_df.annotation == False) & (self.edge_df.edge_type == "intron"),
            "edge_id",
        ]
                    ...
                    for gid in skip_genes:
                        ...
                        for skip_eid in sub_edges.index:
                            ...
                            temp_df = skip_g_df[
                                [
                                    skip_eid in path
                                    for path in skip_g_df.path.values.tolist()
                                ]
                            ]
        ...
        print(
            "Found {} novel es events in {} transcripts.".format(
                len(es_df.index), len(es_transcripts)
            )
        )
    ...
    def die_gene_test(
        ...
        if len(adata.obs[obs_col].unique().tolist()) != 2 and not obs_conditions:
            raise ValueError(
                "Must provide obs_conditions argument when obs_col has"
                " >2 unique values"
            )
        elif not obs_conditions:
        ...
        if obs_col and obs_conditions:
            ...
            for cond in obs_conditions:
                if cond not in conds:
                    raise ValueError(
                        "Value {} not found in metadata column {}.".format(
                            cond, obs_col
                        )
                    )
        ...
        # add gene id
        df = df.merge(ref_df["gid"], how="left", left_index=True, right_index=True)
        ...
        gene_de_df = pd.DataFrame(
            ...
            data=[[np.nan for i in range(10)] for j in range(len(gids))],
        )
        ...
        for gene in gids:
            ...
            gene_df, test_result = get_die_gene_table(
                gene_df, obs_conditions, rc_thresh
            )
        ...
        uns_name = make_uns_key(
            kind=kind, obs_col=obs_col, obs_conditions=obs_conditions
        )
    ...
    def get_die_genes(
        self, kind="iso", obs_col="dataset", obs_conditions=None, p=0.05, dpi=10
    ):
        """
        ...
        # check to see if this was even tested
        uns_name = make_uns_key(
            kind="die", obs_col=obs_col, obs_conditions=obs_conditions, die_kind=kind
        )
        try:
            test = self.adata.uns[uns_name]
        except:
            raise Exception(
                "Problem accessing DIE test results for {}, {}".format(
                    obs_col, obs_conditions
                )
            )
        ...
        for i, group in enumerate(groupby):
            for adata in adatas:
                if i == 0:
                    ...
                    adata.obs[col_name] = (
                        adata.obs[col_name] + "_" + adata.obs[group].astype(str)
                    )
        return col_name
    ...
    def get_edge_abundance(self, prefix=None, kind="counts"):
        ...
        temp = self.edge_df.merge(
            self.loc_df[["chrom", "coord"]],
            how="left",
            left_on="v1",
            right_on="vertex_id",
        )
        ...
        temp = temp.merge(
            self.loc_df[["coord"]], how="left", left_on="v2", right_on="vertex_id"
        )
    ...
    def get_end_abundance(self, prefix, kind, how="tss"):
        ...
        # add location information to end_adata.var
        temp = temp.merge(self.loc_df[["chrom", "coord"]], how="left", on="vertex_id")
    ...
    def set_plotting_colors(self, cmap=None, default=False):
        ...
        def adjust_lightness(color, amount=0.5):
            ...
            c = colorsys.hls_to_rgb(c[0], 0.8, c[2])
            c = mc.to_hex(c)
            return c
    ...
    def plot_graph(
        self, gid, indicate_dataset=False, indicate_novel=False, prefix=None
    ):
        # display=True):
        """
    ...
    def plot_browser(self, tid, **kwargs):
        ...
        self.pg.init_plot_settings(
            self, tid=tid, indicate_dataset=False, indicate_novel=False, browser=True
        )
    ...
    def plot_transcript_path(
        ...
        **kwargs,
        ...
        if prefix:
            fname = create_fname(
                prefix, indicate_dataset, indicate_novel, browser, ftype="path", tid=tid
            )
            self.pg.plot_graph()
            print("Saving transcript path graph for {} as {}".format(tid, fname))
            save_fig(fname)

    def plot_each_transcript(
        self, tids, prefix, indicate_dataset=False, indicate_novel=False, browser=False
    ):
        """
    ...
    def plot_each_transcript_in_gene(
        self, gid, prefix, indicate_dataset=False, indicate_novel=False, browser=False
    ):
        """
    ...
    def gen_report(
        ...
        if transcript_col not in self.t_df.columns:
            raise Exception(
                "Transcript identifier column {} ".format(transcript_col)
        ...
        if novelty:
            if not self.has_novelty():
                raise Exception(
                    "No novelty information present in the graph. "
        ...
        if include_qvals:
            uns_key = make_uns_key(
                kind="det", obs_col=qval_obs_col, obs_conditions=qval_obs_conditions
            )
        ...
        sg.plot_each_transcript(
            report_tids, prefix, indicate_dataset, indicate_novel, browser=browser
        )
        ...
        if layer == "tpm":
            ...
            plt.savefig(
                gid_prefix + "_colorbar_scale.png",
                format="png",
                bbox_inches="tight",
                dpi=200,
            )
        ...
        pdf_name = create_fname(
            prefix, indicate_dataset, indicate_novel, browser, ftype="report", gid=gid
        )
    ...
    def check_plotting_args(self, indicate_dataset, indicate_novel, browser=False):
        ...
        if indicate_dataset and indicate_novel:
            raise Exception(
                "Please choose either indicate_dataset " "or indicate_novel, not both."
            )
        ...
        if browser:
            if indicate_novel or indicate_dataset:
                raise Exception(
                    "Cannot indicate_novel or indicate_dataset " "with browser option."
                )
