...
class Report(FPDF):
    ...
    def header(self):
        ...
        for col in self.datasets:
            ...
            # just using the dataset names
                ...
                self.cell(
                    self.w_dataset, self.header_height, col, border=True, align="C"
                )
        ...
        if self.report_type == "browser":
            self.image(
                self.prefix + "_browser_scale.png",
                x=browser_scale_x,
    ...
    def footer(self):
        if not self.novelty:
            ...
            self.set_x(90.5)
        self.image(self.prefix + "_colorbar_scale.png", w=90, h=13.57)
    ...
    def add_transcript(self, entry, oname, tid):
        ...
        if self.include_qvals:
            qval_entry = (
                self.qval_df.loc[self.qval_df.tid == entry.name].head(1).squeeze()
            )
        ...
        if self.novelty:
            self.cell(25, self.entry_height, entry["novelty"], border=True, align="C")
