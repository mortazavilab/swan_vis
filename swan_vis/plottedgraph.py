...
class PlottedGraph(Graph):
    ...
    def calc_node_colors(self):
        self.loc_df = self.loc_df.apply(lambda x: self.get_node_color(x), axis=1)
    ...
    def is_novel_or_in_dataset(self, x):
        if self.indicate_novel and is_novel(x):
            if self.graph_type == "transcript_path" and x.vertex_id in self.loc_path:
    ...
    def calc_edge_curves(self):
        ...
        edge_dict = {"pos": "arc3,rad=", "neg": "arc3,rad=-", "straight": None}
    ...
    def calc_edge_linestyles(self):
        self.edge_df["line"] = self.edge_df.apply(
            lambda x: self.get_edge_linestyle(x), axis=1
        )
    ...
    def plot_line_bw_regions(
        self, regions, scale, strand, x, y, h, color, ax, linewidth=1
    ):
        """
    ...
    def plot_scale(self, x, y, h, fontsize, ax, **kwargs):
        ...
        regions = [(scale_min - dist, scale_min), (scale_max, scale_max + dist)]

        # regions on either side of the scale and the line
        ax = self.plot_regions(regions, scale, strand, g_min, g_max, x, y, h, "k", ax)
        ax = self.plot_line_bw_regions(
            regions, scale, strand, x, y, h, "k", ax, **kwargs
        )
    ...
    def plot_regions(
        ...
        for v1, v2 in regions:
            ...
            # plot
            rect = pch.Rectangle(
                (x_coord, y_coord), width, h, color=color, edgecolor=None, linewidth=0
            )
            ax.add_patch(rect)
    ...
    def plot_browser(
        self, x=0, y=0, w=14, h=0.1, color=None, linewidth=3, ax=None, **kwargs
    ):
        """
        ...
        # plot exons
        exons = [
            (self.loc_df.loc[v1, "coord"], self.loc_df.loc[v2, "coord"])
            for v1, v2 in zip(loc_path[:-1], loc_path[1:])
        ][::2]
        y_coord = y
        ax = self.plot_regions(exons, scale, strand, g_min, g_max, x, y, h, color, ax)
        ...
        plt.plot(x_coords, [y_line, y_line], color=color, linewidth=linewidth, zorder=1)
    ...
    def plot_browser_path(self, colors=None, ax=None):
        ...
        def get_tick_coords(loc_df, gene_len, exons, g_min, g_max, strand):
            ...
            for v1, v2 in exons:
                ...
                tick_coords = [
                    t for t in tick_coords if t < start - dist or t > stop + dist
                ]
            ...
            if len(exons) == 2 and not tick_coords:
                tick_coords = [
                    (
                        loc_df.loc[exons[0][1], "coord"]
                        + loc_df.loc[exons[1][0], "coord"]
                    )
                    / 2
                ]
    ...
    def plot_browser_scale(self):
        ...
        # start bar
        rect = pch.Rectangle(
            (scale_start, y_coord), width=width, height=height, color="k"
        )
        ax.add_patch(rect)

        # end bar
        rect = pch.Rectangle(
            (scale_end, y_coord), width=width, height=height, color="k"
        )
        ax.add_patch(rect)

        # plot the line in between the bars
        plt.plot(
            [scale_start + width, scale_end - width], [0, 0], color="k", linewidth=2
        )
    ...
    def get_ordered_edges(self):
        self.edge_df["in_transcript"] = self.edge_df.apply(
            lambda x: 1 if x.edge_id in self.edge_path else 0, axis=1
        )
