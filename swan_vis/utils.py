...
def check_dir_loc(loc):
    ...
    if "/" in loc:
        ...
        if not os.path.isdir(d):
            raise Exception(
                "Directory {} is not found. " "Try a different save location".format(d)
            )
...
def check_file_loc(loc, ftype):
    ...
    if not os.path.isfile(loc):
        raise Exception("{} file not found at {}. " "Check path.".format(ftype, loc))
...
def gtf_or_db(fname):
    ...
    if ext == "gtf":
        ...
        raise Exception(
            "File type must be gtf or db. " "Type received is {}".format(ext)
        )
...
def validate_gtf(fname):
    ...
    """
    df = pd.read_csv(
        fname, sep="\t", usecols=[2, 8], comment="#", names=["entry_type", "fields"]
    )
    if df.empty:
    ...
    if missing_fields:
        raise Exception(
            "Last column of GTF is missing entry types {}".format(missing_fields)
        )
...
def get_ends(t_df, kind):
    ...
    df = pd.DataFrame(
        [[tid, path[ind]] for tid, path in zip(t_df.index, t_df.loc_path)]
    )
...
def calc_total_counts(adata, obs_col="dataset", layer="counts"):
    ...
    df = pd.DataFrame.sparse.from_spmatrix(
        data=sparse.csr_matrix(df.values), index=df.index.tolist(), columns=df.columns
    )
...
def get_stable_gid(df, col):
    ...
    df = df.copy(deep=True)
    try:
        df[["temp", "par_region_1", "par_region_2"]] = df[col].str.split(
            "_", n=2, expand=True
        )
        df[col] = df[col].str.split(".", expand=True)[0]
        df[["par_region_1", "par_region_2"]] = df[
            ["par_region_1", "par_region_2"]
        ].fillna("")
...
def parse_db(database, pass_list, observed, include_isms, verbose):
    ...
    gene_annotations = get_annotations(database, "gene", pass_list=gene_pass_list)
    transcript_annotations = get_annotations(
        database, "transcript", pass_list=transcript_pass_list
    )
    exon_annotations = get_annotations(database, "exon")

    # get transcript data from the database
    gene_2_transcripts = get_gene_2_transcripts(database, transcript_pass_list)
    ...
    for gene_ID, transcript_tuples in gene_2_transcripts.items():
        ...
        for transcript_entry in transcript_tuples:
            ...
            if "transcript_name" not in transcript_annotation_dict:
                transcript_annotation_dict[
                    "transcript_name"
                ] = transcript_annotation_dict["transcript_id"]
...
def parse_gtf(gtf_file, include_isms, verbose):
    ...
    def count_transcripts(gtf_file):
        df = pd.read_csv(
            gtf_file, sep="\t", usecols=[2], names=["entry_type"], comment="#"
        )
        df = df.loc[df.entry_type == "transcript"]
        n = len(df.index)
        return n
